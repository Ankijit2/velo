...
const addEmployee = asyncHandler(async (req: Request, res: Response): Promise<any> => {
  ...
  const employees = await Promise.all(
    added_user_id.map(async (userId: string) => {
...
// Delete Employee
const deleteEmployee = asyncHandler(async (req: Request, res: Response): Promise<any> => {
...
// Get All Employees
const getAllEmployees = asyncHandler(async (req: Request, res: Response): Promise<any> => {
  ...
  return res.status(200).json(
    new ApiResponse(200, { admins, superAdmin, users, currentUser }, "Employees fetched successfully")
  );
});
