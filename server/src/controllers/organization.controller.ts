...
import { createOrgSchema } from "../types/types.js";
 ...
 const createOrganization = asyncHandler(async (req:Request, res:Response):Promise<any> => {
...
const getAvailableOrganizations = asyncHandler(async (req:Request, res:Response):Promise<any> => {
    ...
    return res.status(200).json(
        new ApiResponse(200, organizations, "Available organizations fetched successfully")
    )
})

const getPersonalOrganizations = asyncHandler(async (req:Request, res:Response):Promise<any> => {
    const user = req.session
    console.log(user)
    const joinedOrganizations = await prisma.organisation.findMany({
        ...
        include: {// Include all employee details if needed
            created_by: {
                select:{
                    name:true,
                    email:true,
    ...
    return res.status(200).json(
        new ApiResponse(200, { joinedOrganizations, createdOrganizations }, "User's organizations fetched successfully")
    );
})


 const getOrganizationById = asyncHandler(async (req:Request, res:Response):Promise<any> => {
    const { id } = req.params;
...
const updateOrganization = asyncHandler(async (req:Request, res:Response):Promise<any> => {
    const { id } = req.params;
    const { name } = req.body;
    ...
    return res.status(200).json(
        new ApiResponse(200, organization, "Organization deleted successfully")
    );
})

const deleteOrganization = asyncHandler(async (req:Request, res:Response):Promise<any> => {
    const { id } = req.params;
    const user = req.session;
...
const joinOrganization = asyncHandler(async (req: Request, res: Response): Promise<any> => {
    const user = req.session;
